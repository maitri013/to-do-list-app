class WebSite::ToDoListsController < WebSite::BaseController
  ...
  def create
    ...
    if @to_do_list.save
      redirect_to web_site_show_to_do_list_path(@user.id, @to_do_list.id), notice: "To-do list created successfully."
    else
      render action: "new"
    end
  end
  ...
  def update
    ...
    if @to_do_list.update_attributes(to_do_list_params)
      redirect_to web_site_show_to_do_list_path(@user.id, @to_do_list.id), notice: "To-do list updated successfully."
    else
      render action: "edit"
    end
  end
